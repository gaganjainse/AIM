...
def dashboard_page() -> Any:
    ...
    cached = get_cached_dashboard_data(today)
    if cached:
    ...
    try:
        ...
        set_cached_dashboard_data(today, template_data)
        
        return render_template("dashboard.html", **template_data)
    except Exception as e:
        raise DatabaseError(f"Failed to load dashboard: {e}") from e


def invalidate_dashboard() -> None:
    """Invalidate dashboard cache (called when attendance is modified)."""
    invalidate_dashboard_cache()
